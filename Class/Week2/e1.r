...
# 3. Limpieza de datos
# Función sin_valor(dataframe) que desliega cuantos
# valores NA posee cada variable
sin_valor <- function(x) {
  sum <- 0
  for (i in 1:ncol(x))
  {
    cat("En la columna", colnames(x[i]),
        "total de valores NA:", colSums(is.na(x[i])), "\n")
  }
}
sin_valor(data)

# Función en_blanco(dataframe) que desliega cuantos
# valores en blanco posee cada variable
en_blanco <- function(x) {
  sum <- 0
  for (i in 1:ncol(x))
  {
    cat("En la columna", colnames(x[i]),
        "total de valores en blanco:", colSums(x[i] == ""), "\n")
  }
}
en_blanco(data)
...
# b. Verifiquemos ahora en que clase viajaban dichos
# pasajeros y cuanto les costó el boleto
...
# Promedio que costó un boleto por clase (Pclass) y
...
ggplot(data) +
  ...
  geom_smooth(aes(x = Age, y = Fare, color = Pclass),
